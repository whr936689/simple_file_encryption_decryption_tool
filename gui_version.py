...
lab_1=ctk.CTkLabel(root,text="文件解密/解密工具ver0.1",cursor="hand2",font=("UniSun", 20))
lab_1.bind("<Button-1>",lambda e: root.destroy())
lab_1.pack(pady=10)
# 创建一个StringVar变量以存储单选按钮的值
radio_var = ctk.StringVar()
radio_var.set("3")  #默认为3加密文件
grid_frame_Key=ctk.CTkFrame(root)
grid_frame_Key.pack(pady=10)
Label_Key=ctk.CTkLabel(grid_frame_Key,text="密钥/Key:",font=("UniSun",20)).grid(row=0, column=0)
Ent1=ctk.CTkEntry(grid_frame_Key,font=("",20),width=400)   #密钥
Ent1.grid(row=0, column=1)
grid_frame_Loca=ctk.CTkFrame(root)
grid_frame_Loca.pack(pady=10)
Label_Loca=ctk.CTkLabel(grid_frame_Loca,text="文件路径/Location:",font=("UniSun",20)).grid(row=0, column=0)
Ent2=ctk.CTkEntry(grid_frame_Loca,font=("",20),width=280)    #路径
Ent2.grid(row=0, column=1)
btn_cfile=ctk.CTkButton(grid_frame_Loca,text="打开文件",font=("UniSun",20),command=lambda:select_file(Ent2,radio_var.get()))
btn_cfile.grid(row=0, column=2)
btn1=ctk.CTkButton(root,font=("UniSun",20),text="start",height=50,command=lambda:process(int(Ent1.get()),radio_var.get(),Ent2.get(),update_p))   #实现方法1，后面懒得写
btn1.pack(pady=10)
# 创建单选按钮
...
grid_frame_bar=ctk.CTkFrame(root)
...
def update_p(n,status="处理中..."):    #更新进度原点在这里
    ...
    label_status.config(text=status,font=("UniSun", 20))
    root.update()
